...
app.post("/post-job", async (req, res) => {
  ...
  app.get('/job-roles', async(req, res) => {
    try{
      const roles = await query("SELECT DISTINCT jobTitle FROM jobs");
      console.log(roles);
...
app.get("/intern-view-jobs", async (req, res) => {
  try {
    const date = new Date();
    const jobs = await query('SELECT * FROM jobs where lastDate > ?', [date]);
    res.status(200).json(jobs);
  } catch (err) {
    res.status(500).json({ message: "Server error" });
  }
...
//GETTING APPLICATIONS FOR ALL COMPANIES OR PARTICULAR COMPANIES
app.get('/applications', async (req, res) => {
...
//COMPANY HISTORY USING COMPANY ID
...
//SELECT JOBS FROM PARTICULAR Company
...
//STATISTICS FOR APPLIED STUDENTS COUNT ON HR AND SUPERADMIN DASHBOARD
...
app.post("/accept-interns", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ accepted: acceptedInterns });
  } catch (error) {
...
app.get('/quiz-responses/:token', (req, res) => {
  ...
  SELECT 
      ...
      res.grade
  FROM responses r
  JOIN interns i ON r.user_id = i.id
  LEFT JOIN (
